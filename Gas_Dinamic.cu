...
void M_K(vector<Sensor*> Sensors, const double2* s, const double2* u, double* nn1, double3* nn2, double* nn3, //
    double* np1, double3* np2, double* np3)
{
    mutex mut_1;
    mutex* mut;
    mut = new mutex[K];
...
void Fly_exchenge(Sensor* sens, double x_0, double y_0, double z_0, double Vx, double Vy, double Vz, int ind, //
    const double2* s, const double2* u, double mu, double* nn1, double3* nn2, double* nn3, double* np1, double3* np2, double* np3, //
     mutex* mut)
{
    int next = -1;
    int prev = -1;
    ...
    do
    {
        if (Flying_exchange(KSI, Vx, Vy, Vz, X, Y, Z, next, head, prev, mu, I_do, s[head].x, s[head].y, u[head].x, u[head].y,//
            nn1, nn2, nn3, np1, np2, np3, mut[head]) == false)
        {
            break;
        }
        if (KSI < 0.0)
...
bool Flying_exchange(double& KSI, double& Vx, double& Vy, double& Vz, double& X, double& Y,//
    double& Z, int& next, int head, int prev, const double& mu, double& I_do, const double& ro, const double& p, //
    const double& vx, const double& vy, double* nn1, double3* nn2, double* nn3, double* np1, double3* np2, double* np3, mutex& mut)
...
void Flying_exchange2(Sensor* sens, double Vx, double Vy, double Vz, double X, double Y,//
    double Z, int head, int prev, double mu, const double2* s, const double2* u,//
    double* nn1, double3* nn2, double* nn3, double* np1, double3* np2, double* np3, mutex* mut, bool ExCh, const double& Mu)
    // Эта функция обязательно перезаряжает частицу в каждой ячейке. Расщепление по физическим процессам
    // ExCh - была ли уже перезарядка в этой ячейке?
{
    int n = head % N;                                   // номер ячейки по x (от 0)
    int m = (head - n) / N;                             // номер ячейки по y (от 0)
    ...
    double t1, t2, t3, xx, uz, x, y, z, mu2, mu_ex;
    int mode = 0;
    double time = 1000000000;
    int step = 0;
    double t_ex = 0.0;
    ...
    //cout << X << " " << sqrt(kv(Y) + kv(Z)) << " " << mu << " " << num << endl;
    ...
    while (Peresechenie(x0, y0, X, Y, Z, Vx, Vy, Vz, mode, time) == false)
    {
        ...
        if (step > 6)
        {
            cout << "Error  1605sfgsrvgdrg" << endl;
            return;
        }
        double alpha = polar_angle(Y, Z);
        double yy, zz;
    ...
    if (ExCh == false) // Была ли уже перезарядка в этой ячейке
    {
        double alpha = polar_angle(Y, Z);
        xx = sqrt(kvv(Vx - vx, Vy - vy * cos(alpha), Vz - vy * sin(alpha)))/sk;  // Относительная скорость
        uz = sk * (exp(-xx * xx) / sqrtpi + (xx + 1.0 / (2.0 * xx)) * erf(xx));  // Среднее в интеграле 
        double nu_ex = (ro * uz * sigma(uz));
        double kappa = (nu_ex * time)/ Kn;
        t_ex = -(time / kappa) * log(1.0 - sens->MakeRandom() * (1.0 - exp(-kappa))); // Время до перезарядки

        mu_ex = mu * (1.0 - exp(-kappa));

        mu2 = mu - mu_ex;
        double alpha2 = polar_angle(Y + 0.5 * (t_ex)*Vy, Z + 0.5 * (t_ex)*Vz);
        Istochnik(head, t_ex, mu, alpha2, nn1, nn2, nn3, mut, //
        ...
        {
            if (mu_ex >= sens->MakeRandom() * weight * Mu)
            {
                mu_ex = weight * Mu;
                bb = true;
            }
        ...
        if (bb)  // Иначе нужно забить на эти траектории
        {
            ...
            //cout << x << " " << sqrt(kv(y) + kv(z)) << " " << mu_ex << " " << kk <<  endl;

            double alpha = polar_angle(y, z);

            double Vx2, Vy2, Vz2;

            Change_Velosity(sens, vx / sk, vy * cos(alpha) / sk, vy * sin(alpha) / sk,//
                Vx / sk, Vy / sk, Vz / sk, Vx2, Vy2, Vz2);  // здесь подаются  r, theta, phi

            //xx = sqrt(kvv(Vx - vx, Vy - vy * cos(alpha), Vz - vy * sin(alpha))) / sk;  // Относительная скорость
            //uz = exp(-xx * xx) / sqrtpi + (xx + 1.0 / (2.0 * xx)) * erf(xx);  // Среднее в интеграле 
            //double U = sqrt(kvv(Vx / sk - Vx2, Vy / sk - Vy2, Vz / sk - Vz2));

            //double koef = (U * sigma(U) * exp(-kv(Vx2 - vx / sk) - kv(Vy2 - vy * cos(alpha) / sk) - kv(Vz2 - vy * sin(alpha) / sk))) / (pi * sqrtpi * uz * sigma(uz));

            Vx2 = Vx2 * sk;
            Vy2 = Vy2 * sk;
            Vz2 = Vz2 * sk;

            double P = (pow(sk, 5) * pi * sqrtpi * (3.0 + 2.0 * kv(xx)) / (2.0)) / (pi * sqrtpi * uz);
            double P2 = -(pow(sk, 3) * pi * sqrtpi * (3.0 * kv(sk) + 2.0 * (kv(vx) + kv(vy) - kvv(Vx, Vy, Vz))) / (2.0)) / (pi * sqrtpi * uz);

            mut[head].lock();
            np2[head].x += mu_ex * P;
            np2[head].y += mu_ex * P;
            np2[head].z += mu_ex * P;
            np3[head] += 0.5 * mu_ex * P2;
            mut[head].unlock();
            /*mut[head].lock();
            np2[head].x += mu_ex * (Vx - Vx2);
            np2[head].y += mu_ex * (Vy - Vy2);
            np2[head].z += mu_ex * (Vz - Vz2);
            np3[head] += 0.5 * mu_ex * (kvv(Vx, Vy, Vz) - kvv(Vx2, Vy2, Vz2));
            mut[head].unlock();*/

            Flying_exchange2(sens, Vx2, Vy2, Vz2, x, y,//
                z, head, head, mu_ex, s, u,//
                nn1, nn2, nn3, np1, np2, np3, mut, true, Mu);
        }
    }
    ...
    if (mu2 < 0.01 * Mu)
    {
        if (mu2 >= sens->MakeRandom() * 0.01 * Mu)
        {
            mu2 = 0.01 * Mu;
        }
        else
    ...
    double alpha2 = polar_angle(Y - 0.5 * (time - t_ex) * Vy, Z - 0.5 * (time - t_ex) * Vz);

    Istochnik(head, time - t_ex, mu2, alpha2, nn1, nn2, nn3, mut, //
        Vx, Vy, Vz, 0.0, 0.0, 0.0);

    
    if (mode == 1) // Летим направо
    {
            if (n + 1 >= N)  // Заканчиваем
            {
                return;
            }
            else
            {
                Flying_exchange2(sens, Vx, Vy, Vz, X, Y, Z, m * N + n + 1, head, mu2, s, u,//
                    nn1, nn2, nn3, np1, np2, np3, mut, false, Mu);
                return;
            }
    }
    else if (mode == 2) // Если мы прилетели с левой ячейки, то туда лететь не надо
    ...
    else if (mode == 3)
    {
            ...
            {
                Flying_exchange2(sens, Vx, Vy, Vz, X, Y, Z, (m + 1)* N + n, head, mu2, s, u,//
                    nn1, nn2, nn3, np1, np2, np3, mut, false, Mu);
                return;
            }
    }
...
void Istochnik(const int& i, const double& time, const double& mu, const double& alpha, double* nn1, double3* nn2, double* nn3, mutex* mut, //
    const double& Vx, const double& Vy, const double& Vz, //
    const double& Vx_p, const double& Vy_p, const double& Vz_p)
{
    mut[i].lock();
    nn1[i] += time * mu;
    nn2[i].x += time * Vx * mu;
    nn2[i].y += time * (Vy * cos(alpha) + Vz * sin(alpha)) * mu;
    nn2[i].z += time * (-Vy * sin(alpha) + Vz * cos(alpha)) * mu;
    nn3[i] += time * kvv(Vx, Vy, Vz) * mu;
    mut[i].unlock();
}
