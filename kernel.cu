...
__global__ void Ker_Dekard(double2* s, double2* u, double2* s2, double2* u2, double* T, double* T_do, int method)
{
    ...
    if ((n == 0))
    {
        ...
        if ((u_4.x > -0.5))
        {
            u_4.x = -0.5;              // Условие отсоса жидкости
        }
    }
    else
    {
                            ...
                            double* T, double* T_do, int method, int step)
{
    ...
    if ((n == 0))
    {
        ...
        //if ((u_4.x > 0.0))
        //{
        //    u_4.x = 0.0;              // Условие отсоса жидкости
        //}
...
__global__ void Kernel_TVD(double2* s, double2* u, double2* s2, double2* u2, double* T, double* T_do, int method)
{
   ...
   if (dist <= In_ || dist > 0.8) // Жёсткие граничные условия
    ...
    {
        s_2 = s_1;
        //s_2.y = 0.0001;  // Противодавление для отражения волн
        u_2 = u_1;
        //if (u_2.x < 0.0) u_2.x = 0.0;
    }
    else
    {
        s_2 = s[(m)*N + n + 1];
    ...
    {
        s_5 = s_1;
        //s_5.y = 0.0001;  // Противодавление для отражения волн

        u_5 = u_1;
        //if (u_5.y < 0.0) u_5.y = 0.0;
    }
    else
    {
        s_5 = s[(m + 1) * N + n];
    ...
    else if (method == 1)
    {
        if (x + dx / 2.0 < hx)
        {
            tmin = min(tmin, HLL(s12, u12, s21, u21, 1, 0, Ps12, Pu12, dx));
            //tmin = min(tmin, HLLC_2d_Korolkov(s12, u12, s21, u21, 1, 0, Ps12, Pu12, dx, true));
        }
        else
        {
            //tmin = min(tmin, HLLC_Aleksashov(s12, u12, s21, u21, 1, 0, Ps12, Pu12, dx));
            tmin = min(tmin, HLLC_2d_Korolkov(s12, u12, s21, u21, 1, 0, Ps12, Pu12, dx, false));
        }
        if (y - dy / 2.0 < hy)
        {
            tmin = min(tmin, HLL(s13, u13, s31, u31, 0, -1, Ps13, Pu13, dy));
            //tmin = min(tmin, HLLC_2d_Korolkov(s13, u13, s31, u31, 0, -1, Ps13, Pu13, dy, true));
        }
        else
        {
            //tmin = min(tmin, HLLC_Aleksashov(s13, u13, s31, u31, 0, -1, Ps13, Pu13, dy));
            tmin = min(tmin, HLLC_2d_Korolkov(s13, u13, s31, u31, 0, -1, Ps13, Pu13, dy, false));
        }
        if (x - dx / 2.0 < hx)
        {
            tmin = min(tmin, HLL(s14, u14, s41, u41, -1, 0, Ps14, Pu14, dx));
            //tmin = min(tmin, HLLC_2d_Korolkov(s14, u14, s41, u41, -1, 0, Ps14, Pu14, dx, true));
        }
        else
        {
            //tmin = min(tmin, HLLC_Aleksashov(s14, u14, s41, u41, -1, 0, Ps14, Pu14, dx));
            tmin = min(tmin, HLLC_2d_Korolkov(s14, u14, s41, u41, -1, 0, Ps14, Pu14, dx, false));
        }
        if (y + dy / 2.0 < hy)
        {
            tmin = min(tmin, HLL(s15, u15, s51, u51, 0, 1, Ps15, Pu15, dy));
            //tmin = min(tmin, HLLC_2d_Korolkov(s15, u15, s51, u51, 0, 1, Ps15, Pu15, dy, true));
        }
        else
        {
            //tmin = min(tmin, HLLC_Aleksashov(s15, u15, s51, u51, 0, 1, Ps15, Pu15, dy));
    ...
    if (true)
    {
        ...
        u2[index].x = (s[index].x * u[index].x - (*T_do / dV) * PU.x + *T_do * Q2x) / s2[index].x;
        u2[index].y = (s[index].x * u[index].y - (*T_do / dV) * PU.y + *T_do * Q2y) / s2[index].x;
        s2[index].y = (((s[index].y / (ggg - 1) + s[index].x * (u[index].x * u[index].x + u[index].y * u[index].y) * 0.5) - (*T_do / dV) * PS.y //
            + *T_do * Q3) - 0.5 * s2[index].x * (u2[index].x * u2[index].x + u2[index].y * u2[index].y)) * (ggg - 1);
        if (s2[index].y <= 0)
        {
            s2[index].y = 0.000001;
        }
...
void print_file_mini(double2* host_s_p, double2* host_u_p, double* nn1, double3* nn2, double* nn3, string name, double soltime = 0.0)
{
    ...
    fout << "TITLE = \"HP\"  VARIABLES = \"X\", \"Y\", \"Ro\", \"P\", \"Vx\", \"Vy\", \"Mach\", \"T\",\"Ro_H\",\"Vx_H\",\"Vr_H\",\"Vphi_H\",\"T_H\", ZONE T = \"HP\", SOLUTIONTIME = " << soltime << ", N = " << nn * mm //
    ...
    *host_i = 0;
    ...
    for (int k = 0; k < K; k++)  // Заполняем начальные условия
    {
        ...
        if (true)//(dist <= In_)
        {
            ...
            host_s[k] = { 1.0 * In_ * In_ / (dist * dist), 0.05 * pow(In_ / dist, 2.0 * ggg) };
            host_u[k] = { x / dist , y / dist };
            host_s2[k] = { 1.0 * In_ * In_ / (dist * dist), 0.05 * pow(In_ / dist, 2.0 * ggg) };
            host_u2[k] = { x / dist , y / dist };
        }
        else 
        {
    ...
    for (int stepik = 0; stepik < 0; stepik++)
    {
        cout << "START  STEPIK = " << stepik << endl;
        ...
        M_K(Sensors, host_s, host_u, nn1, nn2, nn3, np1, np2, np3);
        string name = "2D_chi_36_kn_0_42__" + to_string(stepik) + ".txt";
        string name2 = "All_SAVE_chi_36_kn_0_42__" + to_string(stepik) + ".txt";
    ...
    print_file_mini(host_s, host_u, nn1, nn2, nn3, np1, np2, np3, name);*/
    //Save_file(host_s, host_u, nn1, nn2, nn3, np1, np2, np3, "Save_istok.txt");
